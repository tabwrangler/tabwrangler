...
const TabManager = {
  ...
  removeWindow(windowId: number) {
    const lockedIds = window.TW.settings.get<Array<number>>("lockedWindowIds");
    delete lockedIds[lockedIds.indexOf(windowId)];
    window.TW.settings.set("lockedWindowIds", lockedIds);
  },
